...
riot.tag2('card-item', '<yield></yield>', 'card-item,[data-is="card-item"]{ position: relative; display: inline-block; margin: 0 auto; padding: 0; }', '', function(opts) {
});
riot.tag2('card-row', '<div class="card-row-wrapper"> <yield></yield> </div>', 'card-row,[data-is="card-row"]{ position: relative; display: grid; grid-template-columns: 1fr; grid-template-rows: 1fr; grid-template-areas: \'card-row-area\'; margin: 0; padding: 5px; padding-right: 10px; width: 100%; min-height: 200px; overflow: hidden; } card-row>.card-row-wrapper,[data-is="card-row"]>.card-row-wrapper{ grid-area: card-row-area; position: absolute; display: flex; flex-wrap: wrap; justify-content: space-between; margin: 0 auto; padding: 0; width: 100%; } card-row[shadow]>.card-row-wrapper,[data-is="card-row"][shadow]>.card-row-wrapper{ border: 1px solid #EEEEEE; box-shadow: var(--card-box-shadow); }', '', function(opts) {
});
